...
| `pem-normalizer`                            | Already byte-identical in both (confirmed 2026-08) — small, crypto-adjacent; belongs beside `gpg-verifier` below rather than in `pipeline-task-ado`                                                                                                          |                                                                                                                                                                                                                                                                                                                                          |
...
with `openpgp` as an optional peer — it is copied three times today. `pem-normalizer` is the same
shape (small, crypto-adjacent, already byte-identical between the two extensions) and moves alongside
it. `hashicorp-gpg-key` stays in the consuming repos. See decision 4.
